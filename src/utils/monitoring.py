...
class PlatformMonitor:
    ...
    def log_training_metrics(self, metrics: Dict[str, float], context: str = "") -> None:
        """
    ...
    def _check_training_anomalies(self, metrics: Dict[str, float], context: str) -> None:
        """Check for training anomalies."""
        # Check for loss spikes
        if "loss" in metrics:
            recent_losses = [
                m.value for m in self.metrics.get_recent(10)
    ...
    def detect_data_drift(
        ...
        for i, feature_name in enumerate(feature_names):
            try:
                ...
                # Record drift metric
                self.record_metric(
                    f"drift.{feature_name}.ks_statistic",
                    float(ks_stat)
                )
    ...
    def model_performance_decay(
        ...
        for metric_name in set(current_metrics.keys()) & set(reference_metrics.keys()):
            ...
            if reference_value != 0:
                decay_ratio = abs(current_value - reference_value) / abs(reference_value)
                decay_scores[metric_name] = decay_ratio
                
                self.record_metric(
                    f"model.{model_version}.decay.{metric_name}",
                    decay_ratio
                )
