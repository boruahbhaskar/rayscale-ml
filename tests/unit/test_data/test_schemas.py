...
def test_feature_schema_invalid_dtype():
    ...
    with pytest.raises(ValidationError, match="Invalid dtype"):
        FeatureSchema(
            name="test", dtype="invalid_dtype", feature_type=FeatureType.NUMERICAL
        )
...
def test_dataset_schema_creation():
    ...
    features = {
        "feature_1": FeatureSchema(
            name="feature_1", dtype="float32", feature_type=FeatureType.NUMERICAL
        ),
        "feature_2": FeatureSchema(
            name="feature_2", dtype="float32", feature_type=FeatureType.NUMERICAL
        ),
    }

    target = FeatureSchema(
        name="target", dtype="int64", feature_type=FeatureType.NUMERICAL
    )
...
def test_dataset_schema_without_target():
    ...
    dataset = DatasetSchema(
        name="unsupervised_dataset", version="1.0.0", features=features
    )
...
def test_validate_dataset_schema_wrong_type():
    ...
    try:
        ...
        assert any(
            word in error_msg.lower() for word in ['type', 'mismatch', 'int64', 'float']
        )
...
def test_training_data_schema_creation():
    ...
    training_data = TrainingDataSchema(
        features=features, targets=targets, feature_names=["f1", "f2", "f3"]
    )
...
def test_training_data_schema_validation():
    ...
    training_data = TrainingDataSchema(features=features, targets=targets)
...
def test_training_data_schema_invalid_features():
    ...
    with pytest.raises(
        ValidationError, match="All feature vectors must have same length"
    ):
...
def test_training_data_schema_target_length_mismatch():
    ...
    with pytest.raises(
        ValidationError, match="Targets length must match features length"
    ):
...
def test_schema_comparison():
    ...
    feature1 = FeatureSchema(
        name="age", dtype="float32", feature_type=FeatureType.NUMERICAL
    )
...
def test_schema_inequality():
    ...
    feature2 = FeatureSchema(
        name="salary", dtype="float32", feature_type=FeatureType.NUMERICAL
    )
...
@pytest.mark.parametrize(
    "dtype", ["float32", "float64", "int32", "int64", "string", "bool"]
)
...
def test_feature_schema_optional_fields():
    ...
    feature = FeatureSchema(
        name="simple_feature", dtype="float32", feature_type=FeatureType.NUMERICAL
    )
