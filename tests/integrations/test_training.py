...
class TestTraining:
    ...
    def test_model_initialization(self):
        ...
        try:
            ...
            model = TabularMLP(
                input_size=10, hidden_sizes=[64, 32], output_size=1  # For regression
            )
        except TypeError as e:
            # Try different parameter combinations
            print(f"Error with first attempt: {e}")
            # Try without output_size
            model = TabularMLP(input_size=10, hidden_sizes=[64, 32])
    ...
    def test_dataset_creation(self):
        ...
        dataset = SimpleDataset(features=self.X, targets=self.y_reg)
    ...
    def test_training_loop_smoke(self):
        ...
        try:
            model = TabularMLP(input_size=10, hidden_sizes=[32, 16], output_size=1)
        except TypeError:
            model = TabularMLP(input_size=10, hidden_sizes=[32, 16])

        # Create dataset and dataloader
        dataset = SimpleDataset(
            features=self.X[:20], targets=self.y_reg[:20]  # Smaller dataset
        )
    ...
    def test_model_save_load(self):
        ...
        loaded_model = TabularMLP(input_size=10, hidden_sizes=[32, 16])
    ...
    def test_forward_pass(self):
        ...
        # Create model with minimal parameters
        try:
            model = TabularMLP(input_size=5, hidden_sizes=[16, 8], output_size=1)
        except TypeError:
            model = TabularMLP(input_size=5, hidden_sizes=[16, 8])
