...
def test_feature_store_write_dataset(feature_store, sample_dataset, temp_dir):
    ...
    metadata_files = list(
        feature_store.metadata_path.glob(f"test_dataset_{version_id}.json")
    )
...
def test_feature_store_write_dataset_with_partitioning(feature_store, sample_dataset):
    ...
    dataset_with_split = sample_dataset.map_batches(add_split, batch_format="pandas")
...
def test_feature_store_read_dataset(feature_store, sample_dataset):
    ...
    version_id = feature_store.write_dataset(
        dataset=sample_dataset, name="read_test", version="v1.0.0"
    )

    # Read it back
    loaded_dataset, metadata = feature_store.read_dataset(
        name="read_test", version=version_id
    )
    ...
    loaded_columns_filtered = {
        col for col in loaded_columns if not (len(col) == 1 and col.isalpha())
    }
...
def test_feature_store_read_latest_version(feature_store, sample_dataset):
    ...
    for i in range(3):
        ...
        modified_dataset = sample_dataset.map_batches(
            lambda batch: batch.assign(new_col=i), batch_format="pandas"
        )

        feature_store.write_dataset(
            dataset=modified_dataset, name="multi_version", version=f"v{i+1}.0.0"
        )

    # Read latest
    loaded_dataset, metadata = feature_store.read_dataset(
        name="multi_version", version="latest"
    )
...
def test_feature_store_get_metadata(feature_store, sample_dataset):
    ...
    version_id = feature_store.write_dataset(
        dataset=sample_dataset, name="metadata_test", version="v1.0.0"
    )
...
def test_feature_store_list_datasets(feature_store, sample_dataset):
    ...
    for name in ["dataset_a", "dataset_b", "dataset_c"]:
        feature_store.write_dataset(dataset=sample_dataset, name=name, version="v1.0.0")
...
def test_feature_store_delete_dataset_version(feature_store, sample_dataset):
    ...
    for i in range(3):
        version_id = feature_store.write_dataset(
            dataset=sample_dataset, name="delete_test", version=f"v{i+1}.0.0"
        )
...
def test_feature_store_delete_all_versions(feature_store, sample_dataset):
    ...
    for i in range(3):
        feature_store.write_dataset(
            dataset=sample_dataset, name="delete_all_test", version=f"v{i+1}.0.0"
        )
...
def test_feature_store_compute_stats(feature_store, sample_dataset):
    ...
    version_id = feature_store.write_dataset(
        dataset=sample_dataset, name="stats_test", compute_stats=True
    )
...
def test_feature_store_with_empty_dataset(feature_store):
    ...
    version_id = feature_store.write_dataset(
        dataset=empty_dataset, name="empty_dataset", compute_stats=False
    )

    # Should still work
    loaded_dataset, metadata = feature_store.read_dataset(
        name="empty_dataset", version=version_id
    )
...
def test_feature_store_version_strategies(feature_store, sample_dataset):
    ...
    version_id = feature_store.write_dataset(
        dataset=sample_dataset, name="uuid_version", version=DatasetVersion.UUID
    )
    ...
    # Test custom version
    version_id = feature_store.write_dataset(
        dataset=sample_dataset, name="custom_version", version="my-custom-version-1.2.3"
    )
...
def test_feature_store_cache_invalidation(feature_store, sample_dataset):
    ...
    # Write dataset
    version_id = feature_store.write_dataset(
        dataset=sample_dataset, name="cache_test", version="v1.0.0"
    )
...
def test_feature_store_concurrent_access(temp_dir, sample_dataset):
    ...
    version_id = store1.write_dataset(
        dataset=sample_dataset, name="concurrent_test", version="v1.0.0"
    )
...
def test_feature_store_metadata_persistence(temp_dir, sample_dataset):
    ...
    store1 = FeatureStore(temp_dir)
    version_id = store1.write_dataset(
        dataset=sample_dataset, name="persistence_test", version="v1.0.0"
    )
...
def test_feature_store_with_large_dataset(feature_store):
    ...
    version_id = feature_store.write_dataset(
        dataset=large_dataset, name="large_dataset", compute_stats=True
    )
...
def test_feature_store_append_metadata(feature_store, sample_dataset):
    ...
    version_id = feature_store.write_dataset(
        ...
        tags={"source": "synthetic", "quality": "high", "purpose": "testing"},
    )
...
def test_feature_store_varying_sizes(feature_store, num_samples):
    ...
    version_id = feature_store.write_dataset(
        dataset=dataset, name=f"size_test_{num_samples}", compute_stats=True
    )

    # Verify
    loaded_dataset, metadata = feature_store.read_dataset(
        f"size_test_{num_samples}", version_id
    )
