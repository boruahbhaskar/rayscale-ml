...
class ModelManager:
    ...
    def load_model(
        ...
        with self._lock:
            try:
                ...
                logger.info(
                    f"Successfully loaded model {model_name} version {version}"
                )
                
            except Exception as e:
                error_msg = f"Failed to load model {model_name} version {version}: {str(e)}"
    ...
    def _get_latest_version(self, model_name: str) -> str:
        ...
        try:
            ...
            if not versions:
                raise ModelLoadingError(f"No local versions found for model {model_name}")
    ...
    def _load_model_metadata(self, model_name: str, version: str) -> Dict[str, Any]:
        ...
        Returns:
            Model metadata.
        """
        metadata_path = self.model_registry_path / model_name / f"v={version}" / "metadata.json"
    ...
    def _get_input_size(self, model: Any) -> Optional[int]:
    ...
    def _get_output_size(self, model: Any) -> Optional[int]:
