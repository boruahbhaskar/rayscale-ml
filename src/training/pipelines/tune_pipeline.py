    ...
    def _create_search_space(self) -> dict[str, Any]:
        ...
        search_space = {
            "learning_rate": tune.loguniform(
                tune_config.get("lr_min", 1e-4), tune_config.get("lr_max", 1e-2)
            ),
            ...
            "dropout_rate": tune.uniform(
                tune_config.get("dropout_min", 0.1), tune_config.get("dropout_max", 0.5)
            ),
            "num_layers": tune.choice(tune_config.get("num_layers", [2, 3, 4])),
        }
        ...
        def trainable(config: dict[str, Any]):
            ...
            for epoch in range(num_epochs):
                ...
                for batch in train_shard.iter_torch_batches(
                    batch_size=config["batch_size"], device=device, dtypes=torch.float32
                ):
