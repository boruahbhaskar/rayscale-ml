...
class Preprocessor:
    ...
    def transform(self, dataset: rd.Dataset) -> rd.Dataset:
        ...
        def apply_transforms(batch: pd.DataFrame) -> pd.DataFrame:
            ...
            for col, col_stats in stats.items():
                if col in batch.columns:
                    ...
                    if clip_outliers:
                        z_scores = np.abs(
                            (batch[col] - col_stats["mean"])
                            / max(col_stats["std"], 1e-8)
                        )
                    ...
                    if scaling_method == "standard":
                        batch[output_col] = (batch[col] - col_stats["mean"]) / max(
                            col_stats["std"], 1e-8
                        )
                    elif scaling_method == "minmax":
                        batch[output_col] = (batch[col] - col_stats["min"]) / max(
                            col_stats["max"] - col_stats["min"], 1e-8
                        )
                    elif scaling_method == "robust":
                        batch[output_col] = (batch[col] - col_stats["q25"]) / max(
                            col_stats["q75"] - col_stats["q25"], 1e-8
                        )
        ...
        transformed = dataset.map_batches(apply_transforms, batch_format="pandas")
...
def preprocess_pipeline(
    input_path: Path, output_path: Path, source_type: str = "parquet", **kwargs
) -> Path:
    """
    ...
    # Create data source
    data_source = create_data_source(source_type=source_type, path=input_path)
    ...
    logger.info(
        f"Preprocessing pipeline completed. " f"Processed data saved to {output_path}"
    )
