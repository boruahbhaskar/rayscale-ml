...
class FeatureStore:
    ...
    def write_dataset(
        ...
        try:
            ...
            if partition_cols:
                # Check partition columns exist
                missing_cols = [
                    col for col in partition_cols if col not in dataset.columns()
                ]
                if missing_cols:
                    raise FeatureStoreError(
                        f"Partition columns not found: {missing_cols}"
                ...
                dataset.write_parquet(str(output_path), partition_cols=partition_cols)
            else:
                dataset.write_parquet(str(output_path))
            ...
            if hasattr(schema, 'names') and hasattr(schema, 'types'):
                ...
                schema_dict = dict.fromkeys(dataset.columns(), "unknown")
    ...
    def get_metadata(self, name: str, version: str) -> DatasetMetadata:
        ...
        if not metadata_file.exists():
            raise FeatureStoreError(f"Metadata not found for {name} version {version}")
        ...
        for dataset_dir in self.base_path.iterdir():
            if dataset_dir.is_dir() and not dataset_dir.name.startswith("."):
                ...
                if versions:
                    ...
                    try:
                        metadata = self.get_metadata(dataset_dir.name, latest_version)
                        datasets.append(
                            {
...
def get_feature_store(base_path: Path | None = None) -> FeatureStore:
    """
