...
class FeatureEngineer:
    ...
    def add_interaction(
        self, feature1: str, feature2: str, output_col: str | None = None
    ) -> "FeatureEngineer":
        """
    ...
    def add_polynomial(
        self, feature: str, degree: int = 2, output_col: str | None = None
    ) -> "FeatureEngineer":
        """
    ...
    def add_log(
        self, feature: str, output_col: str | None = None, epsilon: float = 1e-8
    ) -> "FeatureEngineer":
        """
    ...
    def add_binning(
        ...
        def binning_transform(batch: pd.DataFrame) -> pd.DataFrame:
            batch = batch.copy()
            batch[output_col] = pd.cut(batch[feature], bins=bins, labels=labels)
            return batch
    ...
    def add_standardization(
        self, feature: str, output_col: str | None = None
    ) -> "FeatureEngineer":
        """
    ...
    def transform(self, dataset: rd.Dataset) -> rd.Dataset:
        ...
        for i, transform in enumerate(self.transformations):
            ...
            transformed = transformed.map_batches(transform, batch_format="pandas")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-name", type=str, default="features", help="Output dataset name"
    )
    parser.add_argument("--version", type=str, default="latest", help="Dataset version")

    args = parser.parse_args()

    try:
        version_id = feature_engineering_pipeline(
            input_path=args.input, output_name=args.output_name, version=args.version
        )
        print(
            f"Successfully created feature dataset: {args.output_name} version {version_id}"
        )
