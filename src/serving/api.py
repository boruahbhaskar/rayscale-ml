...
from src.serving.models import ModelLoadingError, get_model_manager
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        model_manager.load_model(model_name="tabular_nn", version="latest")
...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        logger.error(
            f"Request failed: {request.method} {request.url.path} " f"Error: {str(e)}"
        )
        raise
...
async def predict(
    ...
    try:
        ...
        prediction = model_manager.predict(
            features=features, model_name=model_name, version=version
        )
    ...
    try:
        ...
        for features, prediction in zip(request.features, predictions):
            response = PredictionResponse(
                ...
                or model_manager.current_version
                or "unknown",
                prediction_id=str(uuid.uuid4()),
                timestamp=time.strftime("%Y-%m-%dT%H:%M:%SZ"),
            )
...
@app.post("/model/load", summary="Load Model", description="Load a model into memory")
...
@app.post(
    "/model/unload", summary="Unload Model", description="Unload a model from memory"
)
...
async def get_loaded_models(model_manager: get_model_manager_dep = Depends()) -> dict:
    """Get loaded models."""
