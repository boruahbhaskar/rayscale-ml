...
class ParquetDataSource(DataSource):
    ...
    def __init__(self, path: Path, schema_validation: bool = True, **kwargs):
        """
    ...
    def load_data(self) -> rd.Dataset:
        ...
        try:
            dataset = rd.read_parquet(str(self.path), **self.kwargs)
    ...
    def validate(self, dataset: rd.Dataset) -> bool:
        ...
        try:
            ...
            null_counts_df = dataset.map_batches(
                count_nulls_in_batch, batch_format="pandas"
            ).to_pandas()
            ...
            for col in dataset.columns():
                ...
                if null_count > 0:
                    logger.warning(f"Column {col} has {int(null_count)} null values")
...
class SyntheticDataSource(DataSource):
    ...
    def __init__(self, num_samples: int = 10000, num_features: int = 4, seed: int = 42):
        """
    ...
    def load_data(self) -> rd.Dataset:
        ...
        # Create DataFrame
        df = pd.DataFrame(
            features, columns=[f"feature_{i+1}" for i in range(self.num_features)]
        )
    ...
    def validate(self, dataset: rd.Dataset) -> bool:
        ...
        if dataset.count() != self.num_samples:
            logger.warning(
                f"Expected {self.num_samples} samples, " f"got {dataset.count()}"
            )
            return False
